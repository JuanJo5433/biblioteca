...
export function Resume() {
  const { cart, total, setTotal, deleteItem, changeQuantityItems } = useCart();
          ...
          const fetchedBooks = await Promise.all(
            cart.map(async (item) => {
              try {
                ...
                console.error(`Error fetching book with ID ${item.id}:`, error);
  ...
  const cantSelect = (idItem, value) => {
    Number(value)
    const options = [value - 2, value - 1, value, value + 1, value + 2, value + 3, value + 4];
    
    // Manejador de cambios de cantidad
    const handleChange = (event) => changeQuantityItems(idItem, Number(event.target.value));
    ...
    <div className="bg-[var(--background-main)] p-6">
      <div className="my-20 max-w-4xl mx-auto bg-[var(--background-secondary)] rounded-lg shadow-lg">
        ...
        <header className="text-center py-4">
          <h1 className="text-2xl font-bold text-[var(--espresso-brown)]">Tu Carrito</h1>
        </header>

        {/* Resumen del pedido */}
        <div className="bg-[var(--soft-taupe)] p-4 rounded-md">
          <h2 className="font-semibold text-lg text-[var(--text-primary)]">Resumen del Pedido</h2>
        ...
        <table className="w-full mt-4 border-collapse">
          <thead>
            <tr className="text-left border-b border-[var(--border-light)]">
              {["Portada", "Título", "Autor", "Precio", "Cantidad", "Subtotal", ""].map((header) => (
                <th key={header} className="py-2 px-4 text-[var(--text-secondary)]">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {books.map((book) => (
              <tr key={book.id} className="border-b border-[var(--border-light)]">
                ...
                <td className="py-2 px-4 text-[var(--text-primary)]">{book.title}</td>
                <td className="py-2 px-4 text-[var(--text-primary)]">{book.author?.name}</td>
                <td className="py-2 px-4 text-[var(--text-primary)]">${book.purchasePrice}</td>
        ...
        <div className="p-4 bg-[var(--soft-taupe)] flex justify-between items-center">
          <span className="text-lg font-bold text-[var(--text-primary)]">Total: ${total}</span>
